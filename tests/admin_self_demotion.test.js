...
describe("Admin Self-Demotion Prevention", () => {
  ...
  it("should prevent admin from demoting their own role to 'user' or 'publisher'", async () => {
    ...
    expect(errorResponse.message).toMatch(/Admin cannot demote their own account role/i);
  ...
  it("should allow admin to update another user's role", async () => {
    ...
    User.findByIdAndUpdate.mockResolvedValue({ _id: "user456", role: "publisher" });

    await updateUser(req, res, next);

    expect(next).not.toHaveBeenCalled();
    expect(User.findByIdAndUpdate).toHaveBeenCalledWith("user456", { role: "publisher" }, {
      new: true,
      runValidators: true,
    });
  ...
  it("should allow admin to update their own non-role fields (e.g., name)", async () => {
    ...
    User.findByIdAndUpdate.mockResolvedValue({ _id: "admin123", name: "New Admin Name", role: "admin" });

    await updateUser(req, res, next);

    expect(next).not.toHaveBeenCalled();
    expect(User.findByIdAndUpdate).toHaveBeenCalledWith("admin123", { name: "New Admin Name" }, {
      new: true,
      runValidators: true,
    });
    expect(res.status).toHaveBeenCalledWith(200);
